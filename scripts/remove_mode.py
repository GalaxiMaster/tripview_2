...
def remove_mode(path: str = 'assets/'):
    ...
    routes = pd.read_csv(f'{path}routes.csv', dtype=ROUTES_DTYPES)
    ...
    # 2. Filter trips
    trips = pd.read_csv(f'{path}trips.csv', dtype=TRIPS_DTYPES, low_memory=False)
    filtered_trips = trips[~trips['route_id'].isin(unwanted_route_ids)]
    filtered_trips.to_csv(f'{path}trips_cleaned.csv', index=False)
    ...
    for chunk in pd.read_csv(f'{path}stop_times.csv', chunksize=chunk_size, dtype=STIMES_DTYPES):
        filtered_chunk = chunk[chunk['trip_id'].isin(valid_trip_ids)]
        valid_stop_ids.update(filtered_chunk['stop_id'].unique())

        mode = 'w' if first_chunk else 'a'
        filtered_chunk.to_csv(f'{path}stop_times_cleaned.csv', mode=mode, index=False, header=first_chunk)
        first_chunk = False
    ...
    stops = pd.read_csv(f'{path}stops.csv', dtype=STOPS_DTYPES)
    ...
    filtered_stops.to_csv(f'{path}stops_cleaned.csv', index=False)
